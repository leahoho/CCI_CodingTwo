...
ofVec3f Boid::separation(std::vector<Boid *> &otherBoids, std::vector<Boid*> &otherFlock)
{
	...
	{
		if (position.distance(otherFlock[i]->getPosition()) < separationThreshold)
		{
			ofVec3f v = position - otherFlock[i]->getPosition();
			v.normalize();
			return v;
		}
	}
...
void Boid::update(std::vector<Boid *> &otherBoids, std::vector<Boid*> &otherFlock, ofVec3f &min, ofVec3f &max)
{
	velocity += separationWeight*separation(otherBoids, otherFlock);
